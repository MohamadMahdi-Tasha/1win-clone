...
export default function NavLinkComponent({name, isImg = false, imgSrc, isActive, link}:propsType):ReactNode {
    // Returning JSX
    return (
        <li>
            <Link href={link} data-active={isActive} className={`${(isImg) ? 'block lg:w-[50px] w-[100px]' : 'lg:flex block lg:px-0 lg:py-0 lg:rounded-0 px-[10px] py-[5px] rounded-[50rem] justify-center items-center lg:w-[50px]'} lg:data-[active="true"]:bg-transparent data-[active="true"]:bg-blue lg:data-[active="true"]:shadow-none data-[active="true"]:shadow-blueNeon`}>
                {
                    (isImg)
